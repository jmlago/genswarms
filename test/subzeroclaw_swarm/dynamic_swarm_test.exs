defmodule SubzeroclawSwarm.DynamicSwarmTest do
  @moduledoc """
  ...
  alias SubzeroclawSwarm.{SwarmManager, Routing.Router}
  ...
  defp wait_unregistered(swarm, name, attempts \\ 50) do
    case Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, name}) do
      [] -> :ok
      _ when attempts > 0 ->
        Process.sleep(10)
  ...
  describe "add_object/3" do
    test "adds an object and wires up connections", %{swarm: swarm} do
      ...
      assert [_] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, :sink2})
  ...
  describe "remove_object/3" do
    test "removes object and clears edges", %{swarm: swarm} do
      ...
      assert [] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, :scratch})
  ...
  describe "add_agent/3 (with :mock backend)" do
    test "edges are registered before agent is started (no invalid_route race)", %{swarm: swarm} do
      ...
      assert [{_pid, :agent}] =
               Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, :beta})
    end
  ...
  describe "remove_agent/3" do
    test "stops agent and removes from topology", %{swarm: swarm} do
      {:ok, :delta} = SwarmManager.add_agent(swarm, %{name: :delta, backend: :mock},
                                              connections: [:sink])

      :ok = SwarmManager.remove_agent(swarm, :delta)
      :ok = wait_unregistered(swarm, :delta)

      assert [] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, :delta})
  ...
  describe "scale_agent_group/4" do
    test "scales up from 0 to N using existing agent as template", %{swarm: swarm} do
      ...
      for i <- 1..3 do
        assert [_] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, :"alpha_#{i}"})
      end

      # Bare :alpha is gone
      assert [] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, :alpha})
    end
    ...
    test "scales down by removing extras", %{swarm: swarm} do
      ...
      for name <- [:alpha_3, :alpha_4, :alpha_5] do
        assert [] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm, name})
      end
    end
  ...
  describe "ExsWriter snapshot" do
    test "round-trips through Loader", %{swarm: swarm} do
      ...
      assert {:ok, reloaded} = SubzeroclawSwarm.Config.Loader.load(tmp)
  ...
  describe "overlay replay on swarm restart" do
    test "persisted agents are restored after stop+start" do
      ...
      assert [_] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm_name, :extra})
    ...
    test "scale events replay deterministically" do
      ...
      for i <- 1..4 do
        assert [_] = Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm_name, :"worker_#{i}"})
      end
