defmodule Genswarm.DynamicSwarmTest do
  @moduledoc """
  ...
  alias Genswarm.{SwarmManager, Routing.Router}
  ...
  defp wait_unregistered(swarm, name, attempts \\ 50) do
    case Registry.lookup(Genswarm.AgentRegistry, {swarm, name}) do
      [] -> :ok
      _ when attempts > 0 ->
        Process.sleep(10)
  ...
  describe "add_object/3" do
    test "adds an object and wires up connections", %{swarm: swarm} do
      ...
      assert [_] = Registry.lookup(Genswarm.AgentRegistry, {swarm, :sink2})
  ...
  describe "remove_object/3" do
    test "removes object and clears edges", %{swarm: swarm} do
      ...
      assert [] = Registry.lookup(Genswarm.AgentRegistry, {swarm, :scratch})
  ...
  describe "add_agent/3 (with :mock backend)" do
    test "edges are registered before agent is started (no invalid_route race)", %{swarm: swarm} do
      ...
      assert [{_pid, :agent}] =
               Registry.lookup(Genswarm.AgentRegistry, {swarm, :beta})
    end
  ...
  describe "remove_agent/3" do
    test "stops agent and removes from topology", %{swarm: swarm} do
      ...
      assert [] = Registry.lookup(Genswarm.AgentRegistry, {swarm, :delta})
  ...
  describe "scale_agent_group/4" do
    test "scales up from 0 to N using existing agent as template", %{swarm: swarm} do
      ...
      for i <- 1..3 do
        assert [_] = Registry.lookup(Genswarm.AgentRegistry, {swarm, :"alpha_#{i}"})
      end

      # Bare :alpha is gone
      assert [] = Registry.lookup(Genswarm.AgentRegistry, {swarm, :alpha})
    end
    ...
    test "scales down by removing extras", %{swarm: swarm} do
      ...
      for name <- [:alpha_3, :alpha_4, :alpha_5] do
        assert [] = Registry.lookup(Genswarm.AgentRegistry, {swarm, name})
      end
    end
  ...
  describe "ExsWriter snapshot" do
    test "round-trips through Loader", %{swarm: swarm} do
      ...
      source = Genswarm.Config.ExsWriter.to_exs_source(config)
      ...
      assert {:ok, reloaded} = Genswarm.Config.Loader.load(tmp)
  ...
  describe "overlay replay on swarm restart" do
    test "persisted agents are restored after stop+start" do
      ...
      assert [_] = Registry.lookup(Genswarm.AgentRegistry, {swarm_name, :extra})
    ...
    test "scale events replay deterministically" do
      ...
      for i <- 1..4 do
        assert [_] = Registry.lookup(Genswarm.AgentRegistry, {swarm_name, :"worker_#{i}"})
      end
