defmodule SubzeroclawSwarm.SwarmManager do
  ...
  defp replay_overlay(swarm_name, state) do
    events = SubzeroclawSwarm.CLI.SwarmRegistry.load_overlay(swarm_name)
  ...
  defp already_registered?(swarm_name, name) do
    case Registry.lookup(SubzeroclawSwarm.AgentRegistry, {swarm_name, name}) do
      [] -> false
      _ -> true
    end
  end
  ...
  defp maybe_persist(opts, swarm_name, op, payload) do
    if Keyword.get(opts, :persist, false) do
      SubzeroclawSwarm.CLI.SwarmRegistry.append_overlay(swarm_name, op, payload)
    else
      :ok
    end
  end
