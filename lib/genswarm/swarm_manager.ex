defmodule Genswarm.SwarmManager do
  @moduledoc """
  ...
  alias Genswarm.Config.{Loader, SwarmConfig}
  alias Genswarm.Objects.ObjectSupervisor
  ...
  defp replay_overlay(swarm_name, state) do
    events = Genswarm.CLI.SwarmRegistry.load_overlay(swarm_name)
  ...
  defp find_group_members(swarm_name, base_name) do
    ...
    Registry.select(Genswarm.AgentRegistry, [
      {{{swarm_name, :"$1"}, :_, :_}, [], [:"$1"]}
    ])
  ...
  defp already_registered?(swarm_name, name) do
    case Registry.lookup(Genswarm.AgentRegistry, {swarm_name, name}) do
      [] -> false
      _ -> true
    end
  end
  ...
  defp maybe_persist(opts, swarm_name, op, payload) do
    if Keyword.get(opts, :persist, false) do
      Genswarm.CLI.SwarmRegistry.append_overlay(swarm_name, op, payload)
    else
      :ok
    end
  end
