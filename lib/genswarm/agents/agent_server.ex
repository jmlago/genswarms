defmodule Genswarm.Agents.AgentServer do
  @moduledoc """
  ...
  alias Genswarm.Agents.{AgentProtocol, Inbox}
  alias Genswarm.Observability.LogStore
  alias Genswarm.Config.SwarmConfig
  ...
  def via_tuple(swarm_name, agent_name) do
    {:via, Registry, {Genswarm.AgentRegistry, {swarm_name, agent_name}, :agent}}
  end
  ...
  def handle_info(:start_backend, state) do
    ...
    case state.backend_module.start(to_string(state.name), config) do
      {:ok, ref} ->
        ...
        {:ok, watcher} =
          Genswarm.Agents.LogWatcher.start_link(
  ...
  defp prepare_skills(state) do
    skills_base =
      Application.get_env(:genswarm, :swarm_data_dir, "~/.subzeroclaw/swarms")
    ...
    priv_skills = Application.get_env(:genswarm, :skills_dir, "priv/skills")
    project_root = Application.get_env(:genswarm, :project_root) || File.cwd!()
